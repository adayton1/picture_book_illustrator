...
if __name__ == "__main__":
	...
	while True:
		...
		nouns, entities, nouns_to_imgs, template_img_path = find_images_for_page(text, nouns_to_imgs, downloads_dir, nlp, captioner)
		...
		created_image_pil = create_image(nouns, entities, nouns_to_imgs, template_img_path, detector)

		created_image = np.array(created_image_pil)
		created_image = created_image[:, :, ::-1].copy()

		composed_image_path = os.path.join(output_dir, "pages", "{}.jpg".format(pageNum))
