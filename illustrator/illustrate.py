...
def find_noun_images(nlp, text, output_dir):
    ...
    for entity in doc.ents:
        if entity.label_ == "PERSON":
            image_path = google_image_search(entity.label_.lower(), entity.text, output_dir, type="line-drawing")
...
def find_images(text, pages, output_dir):
    ...
    for i, page in enumerate(pages):
        ...
        page_nouns, possible_template_images = find_template_images(doc, os.path.join(output_dir, "templates{0}".format(i)), num_images=1)
...
def pad_bottom_of_images(image_paths, percentage=0.25):
    ...
    for image_path in image_paths:
        ...
        # Pad image
        padded_img = cv2.copyMakeBorder(img, 0, bottom_padding, 0, 0, cv2.BORDER_CONSTANT, value=(255, 255, 255))
...
def add_text_to_images(image_paths, pages, font):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        text_position = compute_text_position(height, width, text_height, image_height)
        d.multiline_text(text_position, multiline_text, font=font, fill="black")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input-file', type=str, required=False, help='Path to the text file.',
                        default=os.path.join(os.path.dirname(__file__), '../data/object_detection_story.txt'))
